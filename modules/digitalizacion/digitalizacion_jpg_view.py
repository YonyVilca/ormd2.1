...
def create_digitalizacion_jpg_view(page: ft.Page, user_data=None):
    ...
    def on_pick(e: ft.FilePickerResultEvent):
        ...
        if added:
            ...
            refresh_table()
            update_ocr_button()
    fp=ft.FilePicker(on_result=on_pick); page.overlay.append(fp)
            ...
            try:
                ...
                if user_row is not None:
                    ...
                    if first_user:
                        ...
                        # No existen usuarios en la BD; no podemos cumplir FKs
                        warn = ft.AlertDialog(
                            title=ft.Text("Usuarios no configurados"),
                            content=ft.Text("No hay usuarios en la base de datos. Crea al menos uno para poder guardar."),
                            actions=[ft.TextButton("Cerrar", on_click=lambda e: page.close(warn))],
                        )
                        page.open(warn)
                        return
            except Exception:
